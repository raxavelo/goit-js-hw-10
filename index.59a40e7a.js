...
const debouncedFetchCountries = (0, _lodash.debounce)(async (value)=>{
    try {
        const countries = await (0, _fetchCountries.fetchCountries)(value);
        if (countries.length > 1) showListOfCountries(countries);
        else showCountry(countries[0]);
    } catch (err) {
        if (err instanceof (0, _limitError.LimitError)) {
            listOfCountries.remove();
            oneCountry.remove();
            (0, _notiflixNotifyAio.Notify).info(err.message);
        } else if (err instanceof (0, _notFoundError.NotFoundError)) {
            listOfCountries.remove();
            oneCountry.remove();
            (0, _notiflixNotifyAio.Notify).failure(err.message);
        } else {
            listOfCountries.remove();
            oneCountry.remove();
            (0, _notiflixNotifyAio.Notify).failure(err.message);
...
function showListOfCountries(list) {
    ...
    list.forEach((country)=>{
        ...
        li.insertAdjacentHTML("afterbegin", `<div><img src="${country.flag}" alt=""><p> - ${country.name}</p></div>`);
...
function showCountry(country) {
    ...
    oneCountry.insertAdjacentHTML("afterbegin", `<h1><img src="${country.flag}" alt=""> ${country.name}</h1>
...
},{}],"1AO7q":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
},{}]},["aP7aF","8lRBv"], "8lRBv", "parcelRequirea64a")
